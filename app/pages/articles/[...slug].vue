...
const breadcrumb = computed(() =>
      mapContentNavigation(findPageBreadcrumb(articleNavigation.value, page.value?.path))
            .map(({ icon, ...link }) => link)
)
...
const articleLink = computed(() => window?.location.href.replace(/\/+$/, ''))
...
<template>
      <UMain class="mt-20 px-2">
            <UContainer class="relative min-h-screen">
                  <UPage v-if="page">
                        <ULink to="/" class="text-sm flex items-center gap-1">
